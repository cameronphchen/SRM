...
do
    ...
    do
        $submittype run_exp.py $dataset $nvoxel $nTR imgpred --loo $loo pca 1 $nfeat --strfresh
        for rand in $(seq 0 4)
        do
            $submittype run_exp.py $dataset $nvoxel $nTR imgpred --loo $loo srm $niter $nfeat -r $rand --strfresh
            $submittype run_exp.py $dataset $nvoxel $nTR imgpred --loo $loo ica      1 $nfeat -r $rand --strfresh
        done
    done
done

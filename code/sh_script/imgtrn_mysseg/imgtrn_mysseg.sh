...
$submittype $exp $dataset $nvoxel $nTR imgtrn_mysseg -e 1st -w $winsize ha_noagg $niter $nvoxel --strfresh
for nfeat in 5 10 50 100 500 #500 #100 #50 #100 500 #1000 #10 50 100 400 813 #500 #813 #50 100 500 1300
do 
    $submittype $exp $dataset $nvoxel $nTR imgtrn_mysseg -e 1st -w $winsize ppca   1 $nfeat --strfresh
    for rand in  $(seq 0 4)
    do
        echo ''
        $submittype $exp $dataset $nvoxel $nTR imgtrn_mysseg  -e 1st -w $winsize pha_em  $niter $nfeat -r $rand --strfresh
        #$submittype $exp $dataset $nvoxel $nTR imgtrn_mysseg  -e 1st -w $winsize spha_vi $niter $nfeat -r $rand --strfresh
        $submittype $exp $dataset $nvoxel $nTR imgtrn_mysseg   -e 1st -w $winsize pica    1 $nfeat -r $rand --strfresh
        #$submittype $exp $dataset $nvoxel $nTR imgtrn_mysseg  -e 1st -w $winsize ha_syn  $niter $nfeat -r $rand --strfresh
        #$submittype $exp $dataset $nvoxel $nTR imgtrn_mysseg  -e 1st -w $winsize ha_syn_noagg  $niter $nfeat -r $rand --strfresh
    done
done

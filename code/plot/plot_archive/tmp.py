...
for i in range(niter/niter_unit):
  ...
  for rnd in range(rand_num):
    ...
    ws_pha_em_rand = np.load(options['working_path']+'HA_rand/HA_rand_'+str(rnd)+'/acc_pHA_EM_rand_'+str(para['nvoxel'])+'vx_'+str(i)+'.npz')
...
#plt.figure()
plt.errorbar(iter_range ,acc_HA_shuffle_mean    ,acc_HA_shuffle_se    , label="HA rand"    , markevery=2,linewidth=2, color='b', marker = 'o')
plt.errorbar(iter_range ,acc_pHA_EM_shuffle_mean,acc_pHA_EM_shuffle_se, label="pHA EM rand", markevery=2, linewidth=2, color='r', marker = 'D',linestyle='--', markersize=7)

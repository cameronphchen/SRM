...
for i in range(1,niter/niter_unit):
  for m in range(nsubjs):
    ...
    for rand in range(rand_num):
      ws_ha_rand     = np.load(options['working_path']+'rand'+str(rand)+'/HA_rand_loo/acc_HA_rand_loo'+str(m)+'_'+str(para['nvoxel'])+'vx_'+str(i)+'.npz')
      ws_pha_em_rand = np.load(options['working_path']+'rand'+str(rand)+'/pHA_EM_rand_loo/acc_pHA_EM_rand_loo'+str(m)+'_'+str(para['nvoxel'])+'vx_'+str(i)+'.npz')

      acc_HA_rand_all[m*rand_num+rand,i]     = ws_ha_rand['accu']
      acc_pHA_EM_rand_all[m*rand_num+rand,i] = ws_pha_em_rand['accu']
      ws_ha_rand.close()
...
acc_HA_rand_mean[0] = acc_None_mean
acc_HA_rand_se [0]  = acc_None_se
acc_pHA_EM_rand_mean[0] = acc_None_mean 
acc_pHA_EM_rand_se[0] = acc_None_se 
...
plt.errorbar(iter_range ,acc_pHA_EM_mean,acc_pHA_EM_se  , label="pHA EM", markevery=2, linewidth=2, color='r', marker = 'D',linestyle='--', markersize=4)

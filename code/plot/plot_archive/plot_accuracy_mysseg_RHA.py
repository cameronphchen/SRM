...
acc_RHA_all = np.zeros((nsubjs*2, niter/niter_unit,11))



for i in range(1,niter/niter_unit):
  for alpha in range(11):
    ws_rha_mysseg_1st = np.load(options['working_path']+'alpha'+str(alpha/10)+'/RHA_mysseg_1st/acc_RHA_mysseg_1st_'+str(para['nvoxel'])+'vx_'+str(i)+'.npz')
    ws_rha_mysseg_2nd = np.load(options['working_path']+'alpha'+str(alpha/10)+'/RHA_mysseg_2nd/acc_RHA_mysseg_2nd_'+str(para['nvoxel'])+'vx_'+str(i)+'.npz')

    acc_RHA_all[range(nsubjs),i,alpha]        = ws_rha_mysseg_1st['accu']
    acc_RHA_all[range(nsubjs,2*nsubjs),i,alpha]   = ws_rha_mysseg_2nd['accu']

    ws_rha_mysseg_1st.close()
    ws_rha_mysseg_2nd.close()
...
for alpha in range(11):
  acc_RHA_all[0:nsubjs,0,alpha]        = ws_none_mysseg_1st['accu'] 
  acc_RHA_all[nsubjs:2*nsubjs,0,alpha] = ws_none_mysseg_2nd['accu'] 

sys.exit()

iter_range = range(niter/niter_unit)
acc_HA_mean = acc_RHA_all.mean(axis = 0).tolist()
acc_HA_se   = acc_RHA_all.std(axis = 0)/math.sqrt(2*nsubjs)
...
plt.errorbar(iter_range ,len(iter_range)*[0.706],\
                         len(iter_range)*[0.026]          , label="Neuron HA"      , markevery=2, linewidth=1, color='k',linestyle='-')
...
plt.legend(loc=4)
plt.savefig(options['output_path']+'accuracy_mysseg_'+str(para['nvoxel'])+'vx.eps', format='eps', dpi=1000,bbox_inches='tight')
